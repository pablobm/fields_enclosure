...
class FormHelperTest < ActionView::TestCase
  ...
  EXPECTED_OUTPUT = "<form action='http://www.example.com' id='create-account' method='post'>" +
  ...
  if (ActionPack::VERSION::STRING.split('.') <=> '2.2'.split('.')) < 0
    def test_form_for_pre_rails_2_1
      _erbout = ''
      ...
      assert_dom_equal EXPECTED_OUTPUT, _erbout
    end
  else
    def test_form_for
      form_for(:account, @account, :html => { :id => 'create-account' }) do |f|
        f.enclosure(:p){ concat f.text_field(:holder_name)}
        f.enclosure(:p){ concat f.text_field(:number)}
        f.enclosure(:p, :class => 'sort_code_triplet') do
          concat f.text_field(:sort_code1)
          concat f.text_field(:sort_code2)
          concat f.text_field(:sort_code3)
        end
        f.enclosure(:p){ concat f.submit('Save account details')}
      end
  
      assert_dom_equal EXPECTED_OUTPUT, output_buffer
    end
  end
