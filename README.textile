...
<pre>
  <%= form_for @thing do |f| %>
    <%= f.enclosure :p %><%= f.text_field :title %><% end %>
    <%= f.enclosure :p %><%= f.text_area  :body %><% end %>
  <% end %>
</pre>
...
<pre><code>
  <form action="/thing" class="new_thing" id="new_thing" method="post">
    <p class="containsErrors"><input id="thing_title" name="thing[title]" size="30" type="text" value="bad value" /></p>
    <p><textarea id="thing_body name="thing[body]">Body of the thing</textarea></p>
  </form>
</code></pre>
